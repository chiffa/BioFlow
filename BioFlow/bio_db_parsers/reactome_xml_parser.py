...
Rna_Collections = {}
# {Id:{'cellularLocation':'', displayName:'', collectionMembers':[], 'references':{'names':[],...}}}
SmallMolecules = {}
...
# {ID:{'product':'','displayName':'','references':{'names':[],...}}}
TemplateReactions = {}
Degradations = {}  # {ID:{'product':'','displayName':'','references':{'eCNumber':[],...}}}
# {ID:{'left':[],'right':[],'displayName':'','references':{'eCNumber':[],...}}}
...
def parse_CellularLocations(root):
    CellularLocationsXml = root.findall(
        ...
        for SmallMolecule_ref_property in single_SmallMolecule_Ref:
            if '}name' in SmallMolecule_ref_property.tag:
                if 'ChEBI:' in SmallMolecule_ref_property.text:
                    ...
                    for word in line:
                        if 'ChEBI:' in word:
                            SmallMoleculeRefs[key]['ChEBI'] = word.split(':')[
                                1].strip(']')
                else:
                    SmallMoleculeRefs[key]['name'].append(
                        SmallMolecule_ref_property.text)
            if '}organism' in SmallMolecule_ref_property.tag and SmallMolecule_ref_property.attrib.values()[
                    0][1:] != 'BioSource1':
                SmallMoleculeRefs[key]['organism'] = BioSources[
                    SmallMolecule_ref_property.attrib.values()[0][1:]]


def parse_ProteinRefs(root):
    ProteinRefsXml = root.findall(
    ...
    for single_ModificationFeature in ModificationFeatureXml:
        ...
        for modification_property in single_ModificationFeature:
            if '}featureLocation' in modification_property.tag:
                ModificationFeatures[key]['location'] = SeqSite[
                    modification_property.attrib.values()[0][1:]]
            if '}modificationType' in modification_property.tag:
                ModificationFeatures[key]['modification'] = SeqModVoc[
                    modification_property.attrib.values()[0][1:]]
...
def parse_Dnas(root):
    ...
    for single_Dna in Dnas_Xml:
        ...
        for dna_property in single_Dna:
            Collection = MetaParser_SecLoop(
                LocalDict, dna_property, Collection)
            if '}entityReference' in dna_property.tag:
                LocalDict['references'] = zipDicts(
                    DnaRefs[
                        dna_property.attrib.values()[0][
                            1:]], LocalDict['references'])
        if Collection:
...
def parse_Rnas(root):
    ...
    for single_Rna in Rnas_Xml:
        ...
        for Rna_property in single_Rna:
            Collection = MetaParser_SecLoop(
                LocalDict, Rna_property, Collection)
...
def parse_SmallMolecules(root):
    SmallMolecules_Xml = root.findall(
    ...
    for single_SmallMolecule in SmallMolecules_Xml:
        ...
        for SmallMolecule_property in single_SmallMolecule:
            Collection = MetaParser_SecLoop(
                LocalDict, SmallMolecule_property, Collection)
...
def parse_Proteins(root):
    ...
    for single_Protein in Proteins_Xml:
        ...
        for Protein_property in single_Protein:
            Collection = MetaParser_SecLoop(
                LocalDict, Protein_property, Collection)
            if '}entityReference' in Protein_property.tag:
                LocalDict['references'] = zipDicts(
                    ProteinRefs[
                        Protein_property.attrib.values()[0][
                            1:]], LocalDict['references'])
            if '}feature' in Protein_property.tag and 'ModificationFeature' in Protein_property.attrib.values()[
                    0]:
                LocalDict['modification'].append(
...
def parse_PhysicalEntities(root):
    PhysicalEntities_Xml = root.findall(
    ...
    for single_PhysicalEntity in PhysicalEntities_Xml:
        ...
        for PhysicalEntity_property in single_PhysicalEntity:
            Collection = MetaParser_SecLoop(
                LocalDict, PhysicalEntity_property, Collection)
        if Collection:
...
def parse_TemplateReactions(root):
    TemplateReaction_Xml = root.findall(
    ...
    for single_Degradation in DegradationXml:
        ...
        for Degradation_property in single_Degradation:
            if '}left' in Degradation_property.tag:
                LocalDict['product'] = Degradation_property.attrib.values()[0][
                    1:]
...
def parse_BiochemicalReactions(root):
    BiochemicalReactionXml = root.findall(
...
def parse_Catalysises(root):
    ...
    for single_Catalysis in CatalysisesXml:
        ...
        for catalysis_property in single_Catalysis:
            if '}controlled' in catalysis_property.tag:
                Catalysises[key]['controlled'] = catalysis_property.attrib.values()[0][
                    1:]
            if '}controller' in catalysis_property.tag:
                Catalysises[key]['controller'] = catalysis_property.attrib.values()[0][
                    1:]
            if '}controlType' in catalysis_property.tag:
                Catalysises[key]['ControlType'] = catalysis_property.text
    TemplateReactRegulationXml = root.findall(
    ...
    for single_TRR in TemplateReactRegulationXml:
        ...
        for TRR_property in single_TRR:
            if '}controlled' in TRR_property.tag:
                Catalysises[key]['controlled'] = TRR_property.attrib.values()[0][
                    1:]
            if '}controller' in TRR_property.tag:
                Catalysises[key]['controller'] = TRR_property.attrib.values()[0][
                    1:]
            if '}controlType' in TRR_property.tag:
                Catalysises[key]['ControlType'] = TRR_property.text
    ControlXml = root.findall(
    ...
    for single_Control in ControlXml:
        ...
        for control_property in single_Control:
            if '}controlled' in control_property.tag and 'Pathway' not in control_property.attrib.values()[
                    0][1:]:
                Catalysises[key]['controlled'] = control_property.attrib.values()[0][
                    1:]
            if '}controller' in control_property.tag:
                Catalysises[key]['controller'] = control_property.attrib.values()[0][
                    1:]
            if '}controlType' in control_property.tag and 'BiochemicalReaction' in control_property.attrib.values()[
                    0]:
                Catalysises[key]['ControlType'] = control_property.text


def parse_Modulations(root):
    ModulationsXml = root.findall(
    ...
    for single_Modulation in ModulationsXml:
        ...
        for modulation_property in single_Modulation:
            ...
            if '}controlled' in modulation_property.tag:
                Modulations[key]['controlled'] = Catalysises[
...
def parse_Pathways(root):
    ...
    for single_Pathway in PathwaysXml:
        ...
        for pathway_property in single_Pathway:
            ...
            if '}pathwayComponent' in pathway_property.tag and 'Pathway' in pathway_property.attrib.values()[
                    0]:
                LocalDict['components'].append(
                    pathway_property.attrib.values()[0][1:])
            if '}pathwayOrder' in pathway_property.tag:
                LocalDict['PathwayStep'].append(
                    pathway_property.attrib.values()[0][1:])
        Pathways[key] = LocalDict


def parse_Pathway_Steps(root):
    Pathway_Steps_Xml = root.findall(
...
if __name__ == "__main__":
    acnums = []
    # TODO: the problem is that here we are performing a zip with Uniprots
